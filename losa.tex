...
\frametitle{Monoid liber}
    \begin{itemize}
        ...
        \item Alfabet: $\Sigma = \{1, 2, 3, 4, 5, 6, 7, 8, 9\}$
        \item Cuvânt: $\alpha = a_1a_2\ldots a_k$ cu $a_i \in \Sigma$
        \item Lungimea cuvântului: $|\alpha| = k$
        \item Concatenarea: $\alpha = a_1a_2\ldots a_k$, $\beta = b_1b_2\ldots b_l$\\
        Atunci $\alpha\cdot\beta = \alpha\beta = a_1a_2\ldots a_kb_1b_2\ldots b_l$
        \item Cuvânt vid: $\varepsilon$ cu $|\varepsilon| = 0$ și $\alpha\varepsilon = \varepsilon\alpha = \alpha \ \forall\alpha$
        \item Mulțimea cuvintelor: \\${\Sigma^* = \{a_1a_2\ldots a_k \mid a_i \in \Sigma, k \in \mathbb{N}\}}$
        \item $(\Sigma^*, \cdot)$ = \textbf{monoidul liber} generat de mulțimea $\Sigma$
    \end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item<1-> $a^ma^n = a^{m+n}$
    \item<2-> Exemplu: $111221 = 1^3 2^2 1^1$ $\rightarrow 312211$
    \item<2-> Exemplu: $33333333333 = 3^{11}$ $\rightarrow 113$ % nu e ok, definim acum unde e ok
\end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item<2-> $LS(777777) = 67$, nu $2747$ sau $5717$.
    \end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item<3-> Ultima cifră a lui $LS^n(S) \ne$ prima cifră a lui $LS^n(D)$, pentru $\forall n \ge n_0$.
    % \item<5-> Notație: $z = x.y$ %adica z se imparte in x si y
    \item<4-> Dacă $y$ se descompune în $x_1$, $x_2$, \ldots, $x_k$, atunci:
    \begin{align*}
        \onslide<4->{LS^n(y) &= \prod_{i=1}^k LS^n(x_i)}
        \onslide<4->{\\ |LS^n(y)| &= \sum_{i=1}^k |LS^n(x_i)|}
    \end{align*}
\end{itemize}
}
\end{frame}
...
Rezultate:
\begin{itemize}
    \item<only@1> Modul în care se repetă începutul și sfârșitul termenilor dintr-un șir look-and-say
    ...
    \item<only@4> \textbf{Rata asimptotică de creștere a lungimii cuvintelor dintr-un șir look-and-say}, \\\vspace{2mm}${\displaystyle \lambda = \lim_{n\to\infty}\frac{|s_{n+1}|}{|s_n|}}$
\end{itemize}
\column{0.4\textwidth}
\centering
\begin{figure}
...
\begin{frame}{Metodă de calcul pentru $\lambda$}
    \only<1>{
        \begin{columns}
            \column{0.5\textwidth}
                \begin{itemize}
                    \item $e_i$ = cele $d$ elemente atomice
                    \item $t_{ij}$ = de câte ori apare $e_i$ în regula de evoluție a lui $e_j$ % adica descompunerea lui $LS(e_j)$
                    \item $f_i$ = de câte ori apare $e_i$ în descompunerea originii șirului
                \end{itemize}
            \column{0.5\textwidth}
                \centering
                \begin{figure}
    ...
    \only<2-8>{
        \begin{itemize}
            \item<2-> Fie $n_0$ astfel încât $LS^{n_0}(s_0) = s_{n_0}$ se descompune în elemente atomice.
            \onslide<3->{\begin{align*}
             |s_{n_0+1}| &= \sum_{i = 1}^{d} (\text{nr. apariții $e_i$ în $LS(s_{n_0})$}) \cdot |e_i|
             \only<4>{\\ &=  \sum_{i = 1}^{d} \left(\sum_{j=1}^d t_{ij}f_j\right) \cdot |e_i| }
             \only<5-6>{\\ &=  \sum_{i = 1}^{d} \left(\textcolor{red}{\sum_{j=1}^d t_{ij}f_j}\right) \cdot |e_i| }
             \only<7->{\\ &= (\mathbf{Tf}) \cdot \mathbf{l} = \mathbf{l}^\top(\mathbf{Tf})}
            \end{align*}
            \vspace{-4mm}
             \only<6-7>{
             \begin{columns}
    ...
    \only<9-12>{
        \begin{itemize}
            \item<9-> $\lambda_i$, $\mathbf{v}_i$ = valori/vectori proprii ai matricei $\mathbf{T}$ % toti, nu doar cei asociati lui lambda
            \item<9-> $\lambda_p$, $\mathbf{v}_p$ = valoare/vector propriu Perron a matricei $\mathbf{T}$
            \item<10-> Pentru simplitate, $n_0 = 0$.
            \item<11-> Când $\mathbf{T}$ e diagonalizabilă,
            \begin{align*}
                \frac{1}{\lambda^n}\mathbf{T}^n\mathbf{f} &= \frac{1}{\lambda^n}\mathbf{T}^n\sum_{i=1}^{d}\alpha_i\mathbf{v}_i
                \onslide<12->{\\ &= \sum_{i=1}^{d}\alpha_i\left(\frac{\lambda_i}{\lambda}\right)^n\mathbf{v}_i}
                \onslide<12->{ \rightarrow \alpha_p \mathbf{v}_p}
            \end{align*} 
        \end{itemize}
    }
    \only<13->{
        \begin{itemize}
            \item<13-> Acum, 
            \begin{align*}
                \only<13>{\lim_{n\to\infty}\frac{|s_{n+1}|}{|s_n|} = \lim_{n\to\infty}\lambda_p\frac{\displaystyle\frac{1}{\lambda_p^{n+1}}\mathbf{T}^{n+1}\mathbf{f} \cdot \mathbf{l}}
                {\displaystyle\frac{1}{\lambda_p^{n}}\mathbf{T}^{n}\mathbf{f} \cdot \mathbf{l}}}
                \onslide<14->{\lim_{n\to\infty}\frac{|s_{n+1}|}{|s_n|} &= \lim_{n\to\infty}\lambda_p\frac{\textcolor{red}{\displaystyle\frac{1}{\lambda_p^{n+1}}\mathbf{T}^{n+1}\mathbf{f}} \cdot \mathbf{l}}
                {\displaystyle \textcolor{red}{\frac{1}{\lambda_p^{n}}\mathbf{T}^{n}\mathbf{f}} \cdot \mathbf{l}}}
                \onslide<15->{\\ &= \lambda_p\frac{\alpha_p \mathbf{v}_p \cdot \mathbf{l}}{\alpha_p \mathbf{v}_p \cdot \mathbf{l}} \\ &= \lambda_p \approx 1.303577269034\ldots}
            \end{align*}
        \end{itemize}
    }
\end{frame}
...
\only<3>{
    \centering
    \includegraphics[scale=0.35]{images/proof222.png} % gemini zice ca limita nu exista
}
...
\only<4>{
    \begin{tabular}{rl}
        1. &\textcolor{red}{i}\\
        5. &\textcolor{red}{ivi}\\
        9. &\textcolor{red}{iviiii}viii\\
        13. &\textcolor{red}{iviiiivv}iiviiiviiiiviiiiiviii\\
        17. &\textcolor{red}{iviiiivviiiv}iiiiviiiiivviivviiviiiviiiivviiviiiviiiiiviiiivivi\\
        21. &\textcolor{red}{iviiiivviiiviiiii}viviivviiiviiiiiviiiiiviviiviiiiivviiviviivv\ldots\\
        25. &\textcolor{red}{iviiiivviiiviiiiivv}iiviiiviiiiiviiiiivviivviiiviiiiviiiviiiii\ldots\\
        29. &\textcolor{red}{iviiiivviiiviiiiivviiiv}iiiiviiiiivviivviiiviiiiiviiiiivviiviv\ldots\\
        33. &\textcolor{red}{iviiiivviiiviiiiivviiiv}iiiiiviviivviiiviiiiiviiiiivviivviiivi\ldots\\
    \end{tabular}
}
\only<5>{
    \centering
    \includegraphics[scale=0.5]{images/proof31.png} % primele 2 incercari empirice, apoi o demonstratie riguroasa dar nu stiu daca e ok? de multe ori zice analizam pe cazuri fara sa faca el analiza pe cazuri, uneori incorect
}
\only<6>{
    \centering
    \includegraphics[scale=0.6]{images/prompt24.png}
}
\only<7>{
    \centering
    \includegraphics[scale=0.35]{images/proof25.png} % demonstreaza ok
}
\end{frame}

\begin{frame}{$LS_{\texttt{sng}}$}
\only<1>{
$LS_{\texttt{sng}}$: 333221, 33221, 23221, 23221, 23221, \ldots
}
\only<2>{
    \centering
    \includegraphics[scale=0.375]{images/proof41.png}
}
\end{frame}
\section{Vă mulțumim!}
\end{document}
